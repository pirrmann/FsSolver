...
let [<Test>] ``Pattern c = var x is promoted to a binding`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings |> shouldEqual (Map.ofList [Local "y", Constant 2M])
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``A constant is moved away from the variable side for addition`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings |> shouldEqual (Map.ofList [Local "x", Computed(0M, ConstValue 1M - ConstValue 1M)])
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``A constant is moved away from the variable side for substraction`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings |> shouldEqual (Map.ofList [Local "x", Computed(2M, ConstValue 1M + ConstValue 1M)])
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``A constant is moved away from the variable side for product`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings |> shouldEqual (Map.ofList [Local "x", Computed(0.5M, ConstValue 1M / ConstValue 2M)])
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``A constant is moved away from the variable side for division`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings |> shouldEqual (Map.ofList [Local "x", Computed(2M, ConstValue 1M * ConstValue 2M)])
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``A division by a variable is not supported by this solver`` () =
    ...
    (fun () -> problem |> Solver.step |> ignore) |> shouldFail<System.Exception>
...
let [<Test>] ``Linear relation with a single variable is solved - solve for net`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "net", Computed(123.044604M, ProvidedValue(123.02M, Local "gross") * (ConstValue 1M + ProvidedValue(2M, Local "execFees") / ConstValue 10000M))
                                    Local "gross" |> ProvidedWith 123.02M
                                    Local "execFees" |> ProvidedWith 2M])
    newProblem.Links
        |> shouldEqual (Set.ofList [Link(Local "net", Local "gross")
...
let [<Test>] ``Linear relation with a single variable is solved - solve for gross`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "net" |> ProvidedWith 123.044604M
...
let [<Test>] ``New bindings are injected in the rules in 2nd step`` () =
    ...
    newProblem.Links
        |> shouldEqual (Set.ofList [Link(Local "x", Local "y")
...
let [<Test>] ``Variables in different scopes are not mixed up`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "net" |> ProvidedWith 2M
                                    Scoped("leg1", Local("net")) |> ProvidedWith 1M
                                    Scoped("leg2", Local("net")), Computed(1M, ProvidedValue(2M, Local "net") - ProvidedValue(1M, ScopedVariable ["net"; "leg1"]))])
    newProblem.Links
        |> shouldEqual (Set.ofList [Link(Local "net", Scoped("leg1", Local("net")))
