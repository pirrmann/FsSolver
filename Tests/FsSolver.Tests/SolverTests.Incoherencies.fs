...
let [<Test>] ``A variable can't have 2 values`` () =
    ...
    newProblem.Bindings |> shouldEqual (Map.ofList [Local "x", Incoherent(LocalVar "x", Conflict([ConstValue 1M; ConstValue 2M]))])
...
let [<Test>] ``Equality of a bound variable to the same value causes no conflict`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings |> shouldEqual problem.Bindings
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``Equality of a bound variable to another value causes a conflict`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "x", Incoherent(ProvidedValue(1M, Local "x"), Conflict([ProvidedValue(1M, Local "x"); ConstValue 2M]))])
    newProblem.Links |> shouldEqual Set.empty
...
let [<Test>] ``Equality of two different variables causes a conflict`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "x", Incoherent(ProvidedValue(1M, Local "x"), Conflict([ProvidedValue(1M, Local "x"); ProvidedValue(2M, Local "y")]))
...
let [<Test>] ``A false relation including two different variables causes a conflict`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "x", Incoherent(ComputedValue(3M, ProvidedValue(1M, Local "x") + ProvidedValue(2M, Local "y")), Conflict([ComputedValue(3M, ProvidedValue(1M, Local "x") + ProvidedValue(2M, Local "y")); ConstValue 1M]))
...
let [<Test>] ``Incoherencies are back propagated to all variables - level 1`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "x", Incoherent(ComputedValue(1M, LocalVar "x"), Conflict([ComputedValue(1M, LocalVar "x"); ComputedValue(2M, LocalVar "y")]))
...
let [<Test>] ``Incoherencies are back propagated to all variables - level 2`` () =
    ...
    newProblem.Rules |> shouldEqual Set.empty
    newProblem.Bindings
        |> shouldEqual (Map.ofList [Local "x", Incoherent(ComputedValue(1M, LocalVar "x"), Propagated)
