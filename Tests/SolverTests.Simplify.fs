...
let [<Test>] ``Absolute values of a positive number is simplified`` () =
    ...
    simplified |> should equal (ComputedValue(1M, expression))
...
let [<Test>] ``Additions are simplified`` () =
    ...
    simplified |> should equal (ComputedValue(2M, expression))
...
let [<Test>] ``Substractions are simplified`` () =
    ...
    simplified |> should equal (ComputedValue(0M, expression))
...
let [<Test>] ``Products are simplified`` () =
    ...
    simplified |> should equal (ComputedValue(6M, expression))
...
let [<Test>] ``Binary MinOf nodes are simplified`` () =
    ...
    simplified |> should equal (ComputedValue(3M, expression))
...
let [<Test>] ``Simplification is applied recursively`` () =
    ...
    simplified |> should equal (ComputedValue(6.5M, expression))
